...
def gripper_full_close(move_client, speed=None):
    """
    [DEPRECATED for probing — use gripper_grasp_close instead]
    يسكر القابض لأقصى حد ممكن (width=0) باستخدام MoveAction.
    لكن MoveAction لا يحافظ على قوة الإغلاق، فالفك ممكن ينفتح تحت الحمل
    الجانبي أثناء probing. تُترك هنا للحالات اللي بدها حركة فقط.
    """
    goal = MoveGoal()
    goal.width = 0.0
...
# قوة افتراضية للـgrasp أثناء probing (نيوتن).
# كافية لتثبيت الفكوك تحت الحمل الجانبي وآمنة للـPanda gripper.
PROBE_GRASP_FORCE = 20.0


def gripper_grasp_close(grasp_client, force=None, speed=None):
    """
    يثبّت الفكوك على بعض بقوة محدّدة باستخدام GraspAction.
    على عكس MoveAction، الـGraspAction يحافظ على قوة الإغلاق بشكل نشط
    فالفك لا ينفتح تحت الحمل الجانبي أثناء probing.
    
    grasp_client يجب أن يكون SimpleActionClient متصل بـ
    /panda1/franka_gripper/grasp (وليس /move).
    """
    if force is None: force = PROBE_GRASP_FORCE
    if speed is None: speed = GRIPPER_SPEED

    goal = GraspGoal()
    goal.width = 0.0
    goal.speed = float(speed)
    goal.force = float(force)
    # epsilon كبير عشان لا يعتبر الإغلاق "فشل" لو الفكوك لمسوا بعض
    # عند width أكبر بشوي من 0 (سماحية ميكانيكية).
...
class BoardCalibration:
    ...
    ملاحظة مهمة: gripper_client الذي يُمرَّر لدوال probing/calibrate
    يجب أن يكون SimpleActionClient لـ GraspAction على
    /panda1/franka_gripper/grasp (وليس /move). هذا ضروري عشان الفكوك
    تبقى مغلقة بقوة 20N أثناء التلامس ولا تنفتح تحت الحمل الجانبي.
    """
    ...
    def _do_probe(self, arm, force_monitor, gripper_client,
        ...
        gripper_client: SimpleActionClient لـ
            /panda1/franka_gripper/grasp (GraspAction).
        """
        rospy.loginfo(f"[Probe] {name}")

        if gripper_client is not None:
            rospy.loginfo("  [probe] grasp-closing gripper before probe "
                          f"(force={PROBE_GRASP_FORCE:.1f} N)")
