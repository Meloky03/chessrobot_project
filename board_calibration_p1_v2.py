...
# -*- coding: utf-8 -*-
"""
board_calibration_p1_v2.py
==========================
معايرة لوحة الشطرنج وبناء مواقع المربعات لذراع Franka Panda.

هذا الإصدار (v2) يحل مشكلة الـ S probing على الروبوت الحقيقي:
- يستعمل **probe S واحد فقط** (بدل اثنين) ⇒ تقليل احتمال الفشل
- يضع الذراع في **joint configuration محدد** قبل الـ S probe
  (configuration "overhead" حيث الذراع ممدود فوق اللوحة)
  ⇒ noise patterns مختلفة ⇒ contact detection أوثق
- e_h يُستنتج من e_N (perpendicular) بدل قياسه من S1+S2
...
import rospy
import moveit_commander  # للـ JointConstraint
import numpy as np
import yaml
import tf2_ros
...
class BoardCalibration:
    ...
    # ------------------------------------------------------------------
    # --- joint-config helpers (جديد في v2) ---
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # --- probe wrapper (نفس الـ logic، بدون joint config) ---
    # ------------------------------------------------------------------
    def _do_probe(self, arm, force_monitor, gripper_client,
                  name, start_xy, direction, probe_yaw,
                  force_threshold=PROBE_FORCE_THRESH):
        """approach + touch + retreat (W probes - بدون joint config محدد).
    ...
    def _do_probe_with_joint_config(self, arm, force_monitor, gripper_client,
        ...
        if elbow_value is None:
            # اقرا elbow الحالي من joint_config
            elbow_value = joint_config[3]   # joint 4 = elbow في Franka Panda
    ...
    def calibrate(self, arm, force_monitor, gripper_client=None, confirm=True):
        rospy.loginfo("=" * 62)
        rospy.loginfo("Starting board calibration v2 (two-pass, S=1 probe)")
