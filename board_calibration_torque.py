...
- USE_WRIST_DEFLECTION_CORRECTION صار ثابت بأول الملف (default=False).
  لتجربة on/off للـoptimization بس غيّر القيمة هنا.
"""
...
PROBE_FORCE_THRESH   = 4   # عتبة |F_xy| لـW probes  (N)
PROBE_FORCE_THRESH_S = 4   # عتبة |F_xy| لـS probes  (N)
...
PROBE_FORCE_THRESH_Z   = 4     # عتبة |F_z|         (N)
PROBE_TORQUE_THRESH    = 2.5   # عتبة max(|τ_ext|) على أي مفصل  (Nm)
PROBE_TORQUE_THRESH_W  = 1.2   # عتبة max(|τ_ext|) على J5/J6/J7 (Nm)
PROBE_INITIAL_SETTLE_S = 0.5   # ثواني نتجاهل فيها أي trigger في بداية الحركة

# =====================================================================
# --- خيارات قابلة للتعديل (للـoptimization tuning) ---
# =====================================================================
# تصحيح انحراف الـwrist (Δyaw) قبل تطبيق gripper tip offset.
# - False (الافتراضي): يستخدم الاتجاه الـnominal للـoffset (legacy).
# - True            : يدور الـoffset بـΔyaw المقاس فعلياً عند كل probe.
# ملاحظة: مع الـmulti-criteria detection بالحالة الحالية، الـcorrection
# مش بيحسن النتيجة فعلياً (Δyaw الجنوبي زحلقة فيزيائية مش deflection).
# مفيد فقط بعد الـoptimization اللي بيقلل الزحلقة بالجنوبي.
...
class BoardCalibration:
    ...
    def _probe_linear(self, move_group, force_monitor, direction_xy,
        ...
        # max trackers — لو فشل الـprobe نطبعهم لمعرفة أي عتبة قريبة من الانطلاق
        fxy_max     = 0.0
        fz_max      = 0.0
        tau_max_max = 0.0
        tau_w_max   = 0.0
        ...
        while not rospy.is_shutdown() and (rospy.Time.now() - t0).to_sec() < timeout:
            ...
            # --- settle window: نتجاهل أول PROBE_INITIAL_SETTLE_S ثواني،
            # أو أول PROBE_MIN_TRAVEL متر، أيهما أطول.
    ...
    def calibrate(self, arm, force_monitor, gripper_client=None, confirm=True):
        ...
        # --- تصحيح انحراف الـwrist (ثابت قابل للتعديل بأول الملف) ---
