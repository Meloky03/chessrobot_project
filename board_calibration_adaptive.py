...
# -*- coding: utf-8 -*-
"""
board_calibration_adaptive.py
=============================
معايرة لوحة الشطرنج وبناء مواقع المربعات لذراع Franka Panda.
نسخة Adaptive Speed - Continuous Motion.

المنطق:
- الذراع يتحرك بشكل مستمر من البداية للنهاية (بدون stop بين الـsegments)
- 3 مستويات سرعة: FAR (2%) -> NEAR (0.5%) -> TOUCH (0.1%)
- لما القوة تعدي عتبة المستوى الجاي، الكود بيعمل replan فوري
  (execute بـwait=False بيستبدل الـtrajectory الحالية بدون stop ملحوظ)
...
ADAPT_SPEED_FAR    = 0.02    # 2%   - بعيد عن الحافة
ADAPT_SPEED_NEAR   = 0.005   # 0.5% - يقترب
ADAPT_SPEED_TOUCH  = 0.001   # 0.1% - يكاد يلامس

ADAPT_THRESH_NEAR  = 0.25    # F > 25% من العتبة -> NEAR
ADAPT_THRESH_TOUCH = 0.60    # F > 60% من العتبة -> TOUCH
...
def get_current_pose_in_ref(move_group, timeout=1.0):
    """ترجع current pose في REFERENCE_FRAME (world)."""
...
def gripper_full_close(move_client, speed=None):
    """يسكر القابض لأقصى حد ممكن قبل الـprobing."""
    goal = MoveGoal()
    goal.width = 0.0
...
def _rot_2d(v, angle):
    """دوران متجه 2D بزاوية angle (CCW)."""
    c, s = np.cos(angle), np.sin(angle)
    return np.array([c*v[0] - s*v[1], s*v[0] + c*v[1]])


def _yaw_from_pose(pose):
    """يستخرج yaw من Pose.orientation (rad)."""
    q = pose.orientation
...
class BoardCalibration:
    """
    حالة المعايرة (corner, axes, theta) + خرائط:
      - square_positions    : a1..h8
      - mirrored_squares    : reversed mapping
      - promotion_positions : q, r, b, n
      - graveyard_positions : 24 موقع
      - hx, hy, hz          : home
    + probing مع adaptive speed continuous + save/load/show/test.
    """
    ...
    # ------------------------------------------------------------------
    # --- probing (Adaptive Speed - Continuous Motion) ---
    # ------------------------------------------------------------------
    def _probe_linear(self, move_group, force_monitor, direction_xy,
        ...
        Probing بسرعة متكيفة بحركة مستمرة:
          - يبدأ trajectory كاملة (max_travel) بسرعة FAR
          - يراقب القوة 200Hz أثناء الحركة
          - لما القوة تعدي مستوى -> replan فوري بسرعة جديدة (بدون stop)
          - الـSTOP الوحيد = عند اللمس الفعلي
        """
        ...
        rospy.loginfo(f"  [probe][DIAG] yaw_start = {np.degrees(yaw_start):+.4f} deg")

        thresh_near  = force_threshold * ADAPT_THRESH_NEAR
        thresh_touch = force_threshold * ADAPT_THRESH_TOUCH

        # ---------------------------------------------------------
        # هيلبر: يخطط trajectory من المكان الحالي للهدف بسرعة محددة
        # ويبدأ تنفيذها بـwait=False (بدون stop ملحوظ)
        # ---------------------------------------------------------
        ...
        while not rospy.is_shutdown() and (rospy.Time.now() - t0).to_sec() < timeout:
            ...
            # replan لما المستوى يتغير (بدون stop)
        ...
        rospy.loginfo(f"  [probe][DIAG] yaw_loaded = {np.degrees(yaw_loaded):+.4f} deg")
        ...
        rospy.loginfo(f"  [probe][DIAG] yaw_settled = {np.degrees(yaw_settled):+.4f} deg")
